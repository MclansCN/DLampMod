...
//                    event.getPlayer().addChatMessage(new TextComponentString("§f【§3次元矿灯§f】§c数据已被清理，若要继续使用其功能，请重新绑定"));
            ...
            try {
                if (!modUpdateInform && DLampMOD.needUpdate) {
                    ...
                    timerswing.schedule(new TimerTask() {
                        @Override
                        public void run() {
                            Minecraft.getMinecraft().player.sendMessage(new TextComponentString("§f【§b次元矿灯MOD§f】§c有更新，请点击§e此处§c下载更新！").setStyle(new Style().setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, DLampMOD.newVersionHomepage))));
                        }
    ...
    public void onLivingHurt(LivingHurtEvent event) {
        if (event.getEntityLiving() instanceof EntityPlayer) {
            ...
            float percent = (event.getEntityLiving().getHealth() - damage) / event.getEntityLiving().getMaxHealth();
            bloodWarn(percent);
        }
    }
